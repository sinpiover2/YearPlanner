...
- **When a "duplicate curriculum" turns out not to be a reliable duplicate, archive it rather than forcing a migration or deletion decision.** A deeper investigation into "which legacy IM1 unit replaces which imported unit" found the mapping wasn't 1:1 by title/content for several units, and one legacy unit had no replacement at all. Rather than resolve every ambiguous case before any progress could be made, reusing an already-established `Active` flag convention (from Sections) let the legacy curriculum become hidden-by-default in the UI immediately, with zero data risk — no migration mapping to get wrong, nothing to delete, nothing teacher-created at risk. Archiving is a strictly safer fallback than deletion or migration whenever the underlying correspondence between old and new data is not fully proven.
- **Reuse an existing schema convention over inventing a new one, even when the task offers several plausible names.** The task suggested `Archived`/`IsArchived`/`Active` as naming options; the codebase already had `Active` on `Sections` with an established default-blank-means-visible convention and a working reader-side helper pattern. Extending that same field name and semantics to `Units` cost less design/review effort and kept the "how do I check if this row is visible" idiom identical across both sheets, rather than introducing a second, oppositely-named flag a future reader would need to remember has different defaults.
