...
- **When a repeated source of friction or confusion is identified, fix the workflow documentation immediately rather than relying on memory or waiting until the project — or even the sprint — is finished.** Sprint 6.6 surfaced this twice: once when decision points kept arriving as open-ended prose instead of a fast lettered choice (fixed by adding User Decision Points to `DEVELOPMENT_WORKFLOW.md`), and again when a sprint started without first confirming the repository, running application, and handoff documentation actually agreed with each other (fixed by adding the Sprint Startup Project Health Check). Neither fix waited for a dedicated cleanup sprint — both were written into the workflow document in the same sitting the friction was noticed, on the reasoning that the workflow itself is a product under continuous improvement, not a fixed process to work around. Treat this as the general pattern: the specific checklist that gets added matters less than not deferring the documentation update past the moment the friction is understood.
