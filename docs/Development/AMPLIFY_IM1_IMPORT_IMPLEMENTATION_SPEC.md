...
Not yet done — remains Sprint 4+: an execute-capable importer (following `ProductionDataCleanup.js`'s guarded preview/execute/backup/lock pattern per §7); reconciling staging IDs with production `UnitID`/`LessonID` conventions; teacher day-entry UI (D-2); `PlannedDays` initialization (D-5); propagating nullable planning values through Forecast's arithmetic; anything touching production data.
