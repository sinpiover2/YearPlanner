...
- The compact preview summary retains validation results, every blocker and warning, exact plan classification counts, teacher-field blocker totals, actual protected legacy-row counts, and the validated payload's extraction-completeness declarations. It omits staged unit/item arrays and all proposed rows so the 8-unit/163-item result remains below the tested 8,000-character logging threshold. The full preview report remains available unchanged in detail.
