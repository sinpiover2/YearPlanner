...
// (blank RequiredDays/OptionalDays), 9 Math 8 units. No Active column yet.
function buildRealShapeUnitsRows() {
  ...
  return [HEADERS, ...legacy.map(row), ...amp.map(row), ...math8.map(row)];
}
...
test("unitsArchiveIsExplicitlyInactive_ only true for explicit falsy values, not blank", () => {
  assert.equal(unitsArchiveIsExplicitlyInactive_(false), true);
  assert.equal(unitsArchiveIsExplicitlyInactive_("false"), true);
  assert.equal(unitsArchiveIsExplicitlyInactive_("False"), true);
  assert.equal(unitsArchiveIsExplicitlyInactive_(""), false);
  assert.equal(unitsArchiveIsExplicitlyInactive_(undefined), false);
  assert.equal(unitsArchiveIsExplicitlyInactive_(true), false);
  assert.equal(unitsArchiveIsExplicitlyInactive_("true"), false);
});

test("plan: real shape, no Active column yet, migration required, safe to execute", () => {
...
test("plan: refuses on unexpected headers (extra/reordered column)", () => {
...
test("plan: with Active column already present, distinguishes already-archived from needs-archiving", () => {
  ...
  // Mark the first target (IM1-U0) already archived.
...
test("plan: alreadyComplete true when Active column present and all 9 targets already archived", () => {
  const finalHeaders = UNITS_ARCHIVE_APPROVED_FINAL_HEADERS;
  const raw = buildRealShapeUnitsRows().map((r) => r.concat([""]));
  raw[0] = finalHeaders;
  const objects = objectsFromRaw(raw);
  objects.forEach((u) => {
    if (UNITS_ARCHIVE_TARGET_UNIT_IDS.includes(u.UnitID)) u.Active = false;
  });
  const plan = unitsArchiveBuildPlan_({ headers: finalHeaders, objects });
...
test("plan: flags a non-target unit that is unexpectedly already Active:false, without blocking targets", () => {
  const finalHeaders = UNITS_ARCHIVE_APPROVED_FINAL_HEADERS;
  const raw = buildRealShapeUnitsRows().map((r) => r.concat([""]));
  raw[0] = finalHeaders;
  const objects = objectsFromRaw(raw);
  const ampUnit = objects.find((u) => u.UnitID === "AMP-IM1-U1");
  ampUnit.Active = false;
  const plan = unitsArchiveBuildPlan_({ headers: finalHeaders, objects });
...
test("unitsArchiveVerify_ passes when all targets archived and non-targets untouched", () => {
  const before = objectsFromRaw(buildRealShapeUnitsRows());
  const after = before.map((u) => {
    const copy = Object.assign({}, u);
    if (UNITS_ARCHIVE_TARGET_UNIT_IDS.includes(u.UnitID)) copy.Active = false;
    return copy;
  });
...
test("unitsArchiveVerify_ fails if a non-target unit's publisher field changed", () => {
  const before = objectsFromRaw(buildRealShapeUnitsRows());
  const after = before.map((u) => {
    const copy = Object.assign({}, u);
    if (UNITS_ARCHIVE_TARGET_UNIT_IDS.includes(u.UnitID)) copy.Active = false;
...
test("unitsArchiveVerify_ fails if a non-target unit was unexpectedly archived", () => {
  ...
  const after = before.map((u) => {
    const copy = Object.assign({}, u);
    if (UNITS_ARCHIVE_TARGET_UNIT_IDS.includes(u.UnitID)) copy.Active = false;
    if (u.UnitID === "M8-U1") copy.Active = false;
    return copy;
  });
  const verification = unitsArchiveVerify_(before, { headers: UNITS_ARCHIVE_APPROVED_FINAL_HEADERS, objects: after });
  assert.equal(verification.valid, false);
...
test("execute: succeeds — adds Active column, archives exactly the 9 targets, leaves AMP/Math8 untouched, verifies, one backup", () => {
  const deps = depsFor(buildRealShapeUnitsRows());
  const report = unitsArchiveExecuteLocked_(UNITS_ARCHIVE_CONFIRMATION_PHRASE, deps);
  ...
  allValues.slice(1).forEach((r) => {
    ...
    if (isTarget) {
      assert.equal(r[activeIndex], false, r[idIndex] + " should be archived");
    } else {
      assert.equal(r[activeIndex], "", r[idIndex] + " should be untouched (blank Active)");
    }
  });
});
...
test("execute: revalidation aborts if a target unit is removed between planning and mutation", () => {
  ...
  deps.spreadsheet.getSheetByName = function (name) {
    if (name === "Units") {
      callCount += 1;
      if (callCount === 2) {
        // Simulate a concurrent edit removing IM1-U8 between the planning
        // pass and the revalidation pass.
